...
import web.project.HouseRentalAPI.service.tenantService;
...
//        return ResponseEntity.ok(newTenant);
        return new ResponseEntity<>(tenantService.addTenant(user), HttpStatus.OK);
    }
...
//            }
//        }
        if (tenantService.existsTenant(request.getEmail())) {
            if (tenantService.confirmPassword(request))
                return new ResponseEntity<>(tenantService.loginTenant(request), HttpStatus.OK);
            return new ResponseEntity<>("Sai mật khẩu", HttpStatus.NOT_FOUND);
        } else return new ResponseEntity<>("Email không tồn tại", HttpStatus.NOT_FOUND);
    }
...
//        return ResponseEntity.status(HttpStatus.OK).body(tenants);
        return new ResponseEntity<>(tenantService.getAllUsers(), HttpStatus.OK);
    }
...
//            return ResponseEntity.status(HttpStatus.NOT_FOUND).body(null);
//        }
        return new ResponseEntity<>(tenantService.searchTenantByName(name), HttpStatus.OK);
    }
...
//            return ResponseEntity.status(HttpStatus.NOT_FOUND).body("Tenant not found");
//        }
        return new ResponseEntity<>(tenantService.rentPaid(id), HttpStatus.OK);
    }
}
